...
type RedisRunner struct {
	...
	// 就绪队列key
	keyReadyQueueHigh string
	keyReadyQueueLow  string

	// prefix
	prefix string
}

func NewRunner(redisCli *redis.Client, threads uint, prefix string, logger *zap.SugaredLogger) (*RedisRunner, error) {
	r := RedisRunner{
	...
	r.keyReadyQueueHigh = r.QueueKey(QueueHigh)
	r.keyReadyQueueLow = r.QueueKey(QueueLow)
	return &r, err
}
...
func (r *RedisRunner) checkWorkingWorkers(ctx context.Context) {
	...
	unlocker, err := RedisLockV(r.redisCli, r.withPrefix(KeyWorkingCheckLocker), r.ID, time.Second*10)
	if err != nil {
...
func (r *RedisRunner) transWaitingWorkers(ctx context.Context) (interval time.Duration) {
	...
	// 获取WaitingQueue独占锁
	unlocker, err := RedisLockV(r.redisCli, r.withPrefix(KeyWaitingQueueLocker), r.ID, WaitingQueueLockTerm)
	if err != nil {
		if err == ErrLockerAlreadySet {
...
func (r *RedisRunner) loadReadyWorkers() (ws []string, err error) {
	unlocker, err := RedisLockV(r.redisCli, r.withPrefix(KeyReadyQueueLocker), r.ID, ReadyQueueLockTerm)
	if err != nil {
	...
	var w []string
	if w, err = r.loadAndTransReadyToWorking(r.keyReadyQueueHigh, highCount); err != nil {
		return
	}
	ws = append(ws, w...)
	if w, err = r.loadAndTransReadyToWorking(r.keyReadyQueueLow, lowCount); err != nil {
		return
	}
	ws = append(ws, w...)
	r.batchPull <- len(ws)
...
func (r *RedisRunner) loadAndTransReadyToWorking(queue string, count int64) (ws []string, err error) {
	...
	_, err = r.redisCli.HSet(ctx, r.withPrefix(KeyWorking), workingVal).Result()
	if err != nil {
		return
	}
...
func (r *RedisRunner) toExec(ws []string) {
	...
	workContents := []string{}
	for _, workerID := range ws {
		pip.HGet(ctx, r.withPrefix(KeyWorkers), workerID)
	}
	rs, err := pip.Exec(ctx)
	if err != nil {
...
func (r *RedisRunner) shouldBachPullReadyCount() (queueHighCount, queueLowCount int64, err error) {
	queueHighLen, err := r.GetQueueLen(r.keyReadyQueueHigh)
	if err != nil {
		return
	}
	queueLowLen, err := r.GetQueueLen(r.keyReadyQueueLow)
	if err != nil {
		return
	}
...
func (r *RedisRunner) recoverWorker(workerID string) error {
	ctx, cancel := context.WithTimeout(context.Background(), RedisTimeout)
	defer cancel()
	_, err := r.redisCli.RPush(ctx, r.keyReadyQueueLow, workerID).Result()
	if err != nil {
		return err
	}
	_, err = r.redisCli.HDel(ctx, r.withPrefix(KeyWorking), workerID).Result()
	if err != nil {
		return err
	}
	return nil
...
func (r *RedisRunner) retryWorker(wc *Meta) {
	...
	if err = r.doSubmit(wc); err == nil {
		ctx, cancel := context.WithTimeout(context.Background(), RedisTimeout)
		defer cancel()
		r.redisCli.HDel(ctx, r.withPrefix(KeyWorking), wc.ID)
	}
	if err != nil {
...
func (r *RedisRunner) removeWorker(wc *Meta) {
	ctx, cancel := context.WithTimeout(context.Background(), RedisTimeout)
	defer cancel()
	if err := r.redisCli.HDel(ctx, r.withPrefix(KeyWorking), wc.ID).Err(); err != nil {
		r.l.Errorf("%s unable to remove worker, err: %v", wc, err)
	}
	if err := r.redisCli.HDel(ctx, r.withPrefix(KeyWorkers), wc.ID).Err(); err != nil {
		r.l.Errorf("%s unable to remove worker, err: %v", wc, err)
	}
}
...
func (r *RedisRunner) newExecWorkerFunc() func(item interface{}) {
	return func(item interface{}) {
		...
		if err = worker.Perform(ctx, r.l.With("workerID", wc.ID)); err != nil {
			r.l.WithOptions(zap.AddCallerSkip(1)).Errorf("%s perform worker err: %v", wc, err)
			wc.Error = err.Error()
			return
		}
		wc.Success = true
...
func (r *RedisRunner) doSubmit(c *Meta) error {
	...
	pip.HSet(ctx, r.withPrefix(KeyWorkers), c.ID, raw)
	queue := r.QueueKey(c.Queue)
	if c.PerformAt != nil && c.PerformAt.After(time.Now()) {
		val := queue + WaitingQueueDataIDSeparator + c.ID
		pip.ZAdd(ctx, r.withPrefix(KeyWaitingQueue), &redis.Z{
...
func (r *RedisRunner) doSetAlive() {
	...
	defer cancel()
	err := r.redisCli.Set(c, r.withPrefix(KeyRunnerAlivePrefix+r.ID), "alive", RunnerAliveStatusTTL*time.Second).Err()
	if err != nil {
...
func (r *RedisRunner) getAllWorkingWorkers() (workers map[string]string, err error) {
	c, cancel := context.WithTimeout(context.Background(), RedisTimeout)
	defer cancel()
	workers, err = r.redisCli.HGetAll(c, r.withPrefix(KeyWorking)).Result()
	if err != nil {
...
func (r *RedisRunner) checkRunnerAlive(runnerID string) (bool, error) {
	c, cancel := context.WithTimeout(context.Background(), RedisTimeout)
	defer cancel()
	val, err := r.redisCli.Get(c, r.withPrefix(KeyRunnerAlivePrefix+runnerID)).Result()
...
func (r *RedisRunner) loadWaitingWorkers(endAt int64) (ws []string, err error) {
	c, cancel := context.WithTimeout(context.Background(), RedisTimeout)
	defer cancel()
	ws, err = r.redisCli.ZRangeByScore(c, r.withPrefix(KeyWaitingQueue), &redis.ZRangeBy{
		Min:    "0",
...
func (r *RedisRunner) transWaitingToReady(ws []string) error {
	...
	for _, w := range ws {
		...
		pip.ZRem(c, r.withPrefix(KeyWaitingQueue), w)
	}
	_, err := pip.Exec(c)
	return err
}
...
func (r *RedisRunner) QueueKey(queue string) string {
	return r.withPrefix("Queue_" + queue)
}

func (r *RedisRunner) withPrefix(key string) string {
	if r.prefix == "" {
		return key
	}
